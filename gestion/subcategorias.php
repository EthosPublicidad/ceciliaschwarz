...
$subcategorias = new GestionSubcategoria();
...
?>
								...
								<div class="modal-content">
									<form class="formModal" action="gestion.php" method="post">
										...
										<div class="modal-footer">
											...
											<input type="hidden" name="opcion" value="altasubcategoria" />
                    ...
                    <div class="col-lg-12">
						<table class="table text-center" id="tabla">
							...
							<tbody>
								<?php
								$regsubcategorias = $subcategorias->traerSubcategorias();
	
								foreach ($regsubcategorias as $reg)
								{
									echo "
									<tr>
										...
										if($reg['estado']=='B'){
											echo "<td>
												<a title='Activar' class='btn btn-primary' href='./gestion.php?id=$reg[id]&opcion=activarsubcategoria'><i class='fa fa-spin fa-refresh'></i></a>
											</td>";
										}else{
											echo "<td>
												<button title='Editar' class='btn btn-success' onclick='editar(\"$reg[descripcion]\", \"$reg[categoria]\", \"$reg[id]\");'><i class='fa fa-pencil'></i></button>
												<a title='Borrar' class='btn btn-danger' href='./gestion.php?id=$reg[id]&opcion=bajasubcategoria'><i class='fa fa-times'></i></a>
											</td>";
										}
									echo "</tr>";
								}
...
<!-- Modal -->
				...
				<form autocomplete="off" class="formModal" action="gestion.php" method="post">
					...
					<div class="modal-body">
						...
						<div style="text-align: left;">
							...
							<?php
							$regcategorias = $subcategorias->traerSubcategorias();
							foreach ($regcategorias as $reg)
							{
								echo "<option value='$reg[id]'>$reg[descripcion]</option>";
							}
					...
					<div class="modal-footer">
						<input type="submit" class="btn btn-primary" value="Continuar" />
						<input type="hidden" name="opcion" value="editarsubcategoria" />
	...
	function editar(descripcion, categoria, id){
